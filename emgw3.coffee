...
FLASH_UBOOT = 'Update the bootloader. You can perform this step in two ways: a) - from your development PC or b) - from the host OS when running balenaOS on your board. If you want to update it from your development PC, you will find the imx-boot binary in the resin-boot partition (1st partition) of the downloaded BalenaOS image. You then need to expose over USB to your development machine the mmcblk1boot0 partition from an u-boot shell on the board with the command: `ums 1 mmc 0.1`. Then on your development machine, make note on the new device node exposed (e.g. /dev/sdX), and flash the bootloader with the command: `sudo dd if=imx-boot of=/dev/sdX seek=33 bs=1024`. To update the bootloader from the host OS you would run the following command: `echo 0 > /sys/block/mmcblk1boot0/force_ro && dd if=/mnt/boot/imx-boot of=/dev/mmcblk1boot0 seek=33 bs=1024`'
