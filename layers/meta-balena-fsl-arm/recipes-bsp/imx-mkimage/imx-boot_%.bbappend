# NOTE: we add a prebuilt imx-boot (taken from OS release v2.75.0+rev1 and based on u-boot version 2020.10) because the imx-boot generated by the compilation
# from source of u-boot 2020.10 we added in our layer in branch 2.51.x fails to boot due to the various other BSP updates we did not backport into our layers.
FILESEXTRAPATHS_prepend := "${THISDIR}/files:"

SRC_URI_append = " file://imx-boot-2.75.0+rev1-prod"
...
do_deploy_append() {
    install -m 0644 ${WORKDIR}/imx-boot-2.75.0+rev1-prod ${DEPLOYDIR}/${BOOT_CONFIG_MACHINE}-${target}
}
